...
export default function Modal() {
  ...
  return (
    <div className=" w-full h-[800px] grid grid-cols-1 md:grid-cols-2">
      <div className=" w-full h-0 md:h-full flex items-start justify-center ">
        <button
          onClick={() => setOpen(true)}
          className={` ${
            open ? "hidden" : " p-2 border-blue-500"
          } p-2 bg-blue-500 rounded-lg text-white border-blue-500 mt-12`}
        >
          Shopping Cart
        </button>
      </div>
      <div className={` ${open ? "":"hidden"} flex w-full max-w-3xl transform text-left text-base transition sm:my-0 `}>
        <form className="relative flex w-full flex-col overflow-hidden bg-white pb-8 pt-0 sm:rounded-lg sm:pb-6 lg:py-8">
          ...
          <section className={` ` } aria-labelledby="cart-heading ">
          ...
          <div className="mt-8 flex justify-end rounded-lg px-4 sm:px-6 lg:px-8 ">
           <div className="bg-indigo-600 rounded-md">
           <button
              type="submit"
              className="rounded-md border border-transparent  px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 focus:ring-offset-gray-50"
            >
              Continue to Payment
            </button>
           </div>
