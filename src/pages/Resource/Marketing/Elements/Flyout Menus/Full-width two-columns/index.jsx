...
export default function Full_width_two_columns() {
  const [open, setOpen] = useState(false);
  const handelClick=()=>{
    setOpen(!open)
  }
  return (
    <div className=" bg-red-500 mb-20 shadow">
      <div className=" bg-white py-5">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <button onClick={ handelClick} className="inline-flex items-center gap-x-1 text-sm font-semibold leading-6 text-gray-900">
            Solutions
      ...
      <div className={`${open ? "block":" hidden"}`} >
        <di className=" inline-block bg-white pt-16 shadow-lg ring-1 ring-gray-900/5">
          <div className="mx-auto grid max-w-7xl grid-cols-1 gap-x-8 gap-y-10 px-6 py-10 lg:grid-cols-2 lg:px-8">
            ...
            <div className="grid grid-cols-1 gap-10 sm:gap-8 lg:grid-cols-2">
              ...
              {recentPosts.map((post) => (
                <article
                  key={post.id}
                  className="  flex max-w-2xl flex-col gap-x-8 gap-y-6 sm:flex-row sm:items-start lg:flex-col lg:items-stretch"
                >
                  <div className=" flex-none">
                    <img
                      ...
                      alt=""
                    />
                    <div className=" inset-0 rounded-lg ring-1 ring-inset ring-gray-900/10" />
                  </div>
                  <div>
                    <div className="flex items-center gap-x-4">
                      ...
                      <a
                        ...
                        className=" z-10 rounded-full bg-gray-50 px-3 py-1.5 text-xs font-medium text-gray-600 hover:bg-gray-100"
                      >
                        {post.category.title}
                      </a>
                    </div>
